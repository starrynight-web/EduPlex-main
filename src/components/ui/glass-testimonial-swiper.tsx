...
// --- Updated Interface: use iconName (string) ---
export interface Testimonial {
  ...
  stats: { iconName: string; text: string }[]; // ← changed from icon: Component
...
// Map icon names to actual Lucide components (only used on client)
    ...
    <section className="testimonials-stack relative pb-10">
      {testimonials.map((testimonial, index) => {
        ...
        return (
          <div
            ref={(el) => {
              cardRefs.current[index] = el; // ✅ returns void implicitly
            }}
            key={testimonial.id}
